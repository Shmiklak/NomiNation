...
class Beatmap extends Model
{
    ...
    public function updateStatus() {
        ...
        $ranked_responses = $this->responses()->where('status', 'RANKED')->count();
        ...
        if ($ranked_responses > 0) {
            $this->status = 'RANKED';
            $this->is_ranked = 1;
            $this->ranked_at = Carbon::now();
        } elseif ($nominated_responses > 0) {
            $this->status = 'NOMINATED';
            $this->is_ranked = 0;
        } elseif ($accepted_responses > 0) {
            $this->status = 'ACCEPTED';
            $this->is_ranked = 0;
        } elseif ($invalid_responses >= 1) {
            $this->status = 'INVALID';
            $this->is_ranked = 0;
        } elseif (
            $uninterested_responses >= $this->queue->not_interested_requirement ||
            ($this->queue->type == 'personal' && $uninterested_responses > 0)
        ) {
            $this->status = 'HIDDEN';
            $this->is_ranked = 0;
        } else {
