...
export default function BeatmapsListing({ paginatedData, members, display_queue = false } : { paginatedData : PaginatedData<Beatmap>, members : User[], display_queue: boolean }) {
    return (
        <>
                    ...
                    <>
                        <div className="grid items-center gap-8 bg-muted-2 lg:grid-cols-4">
                            { paginatedData.data?.map((beatmap: Beatmap, index: number) => (
                                <BeatmapListItem beatmap={beatmap} members={members} key={index} display_queue={display_queue} />
                            )) }
                        </div>

                        <DefaultPagination data={paginatedData}/>
                    </>
            )}
        </>
    )
